...
namespace SimpleSyslog
{
    public static class Syslog
    {
        ...
        public static void Initialize(string hostName, int port, int facility = 16, string sender = null, bool useUtc = false)
        {
            fixedSender = sender;
        ...
        private static void Send(LogLevel logLevel, string message, string sender)
        {
            ...
            message = string.Format("<{0}>{1} {2} {3} {4}", facility * 8 + (int)logLevel, _useUtc ? DateTime.UtcNow.ToString("s") : DateTime.Now.ToString("s"), Dns.GetHostName(), sender, message);
